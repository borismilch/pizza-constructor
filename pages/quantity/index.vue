<template>
  ...
  <div class="qu_container" v-else>
    ...
    <div class="pizza-list" v-if="(pizzas || []).length">
      <PizzaCard v-for="(pizza, idx) in pizzas" :idx="idx" :key="pizza.id" :pizza="pizza" />
    </div>
    ...
    <div class="flex">
      <Addition :addition="add" :key="add.code" v-for="add in additions"/>
    </div>
...
export default class PageQuantity extends Vue {
  ...
  head() {
    return { title: 'Quantity | VuePizza' }
  }
